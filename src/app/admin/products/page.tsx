...
export default function AdminProductsPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#e7e9ee] pb-5">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[#101114] font-serif">
            Home Models & Product CMS
          </h1>
          <p className="text-xs sm:text-sm text-[#6b7280] mt-1 font-medium">
            Manage your catalog of precision steel modular homes, engineering ratings, gallery images, and floor plans.
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-[18px] border border-[#e7e9ee] shadow-[0_12px_35px_rgba(16,24,40,0.04)] flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#6b7280] font-bold">Category:</span>
            <select
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#6b7280] font-bold">Status:</span>
            <select
              value={publishFilter}
      ...
      <div className="bg-white rounded-[18px] border border-[#e7e9ee] shadow-[0_12px_35px_rgba(16,24,40,0.04)] overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center text-[#6b7280] text-xs font-medium">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-[#f6f7f9] border-b border-[#e7e9ee] text-[#101114] font-bold uppercase tracking-wider text-[11px]">
                  <th className="py-4 px-4">Model / Preview</th>
              ...
              <tbody className="divide-y divide-[#e7e9ee]">
                {products.map((prod) => {
                  ...
                  return (
                    <tr key={prodId} className="hover:bg-[#f6f7f9]/70 transition-colors">
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className="relative w-14 h-14 rounded-xl overflow-hidden bg-[#f6f7f9] shrink-0 border border-[#e7e9ee]">
                            <Image
                          ...
                          <div>
                            <div className="font-bold text-[#101114] text-sm">{prod.name}</div>
                            <div className="text-[11px] font-mono text-[#6b7280]">/models/{prod.slug}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-4">
                        <span className="px-2.5 py-1 rounded-md bg-[#f6f7f9] text-[#101114] border border-[#e7e9ee] font-bold text-[11px]">
                          {prod.category}
                        </span>
                      </td>
                      <td className="py-4 px-4 text-[#6b7280] text-xs font-medium">
                        {prod.sqft?.toLocaleString()} sq ft • {prod.bedrooms} Bed, {prod.bathrooms} Bath
                      </td>
                      <td className="py-4 px-4 font-bold text-[#101114] text-sm font-serif">
